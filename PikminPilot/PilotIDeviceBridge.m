...
// Stage 11.6.2 iOS 27+ device-initiated RPPairing responder. Bonjour is
...
static NSString *gPPPairingHostStatus = @"STAGE 11.6.2 PAIRING HOST • idle";
...
static void PPPairingHostReady(const char *serviceIdentifierCString, uint16_t port, const char *txtCString) {
    ...
    if (serviceIdentifier.length == 0) {
        PPSetPairingHostStatus(@"STAGE 11.6.2 PAIRING HOST FAILED • invalid Bonjour service identifier");
        return;
    }
    ...
    PPSetPairingHostStatus([NSString stringWithFormat:
        @"STAGE 11.6.2 PAIRING HOST READY ✅ • port=%u • open Settings → Privacy & Security → Developer Mode → Pair with Pikmin Pilot",
        port]);
}
...
static void PPPairingHostPin(const char *pinCString) {
    ...
    PPSetPairingHostStatus([NSString stringWithFormat:
        @"STAGE 11.6.2 PAIRING PIN • %@ • enter this 6-digit code in the iOS pairing sheet",
        displayPin]);
...
int32_t PPStartPhoneLocalPairingHost(
    ...
    if (outputPath == NULL) {
        PPWriteMessage(message, messageCapacity, @"STAGE 11.6.2 PAIRING HOST FAILED • missing output path");
        return -1;
    }

    PPStopPairingBonjour();
    PPSetPairingHostStatus(@"STAGE 11.6.2 PAIRING HOST • starting TCP responder + Bonjour…");
    ...
    PPStopPairingBonjour();
    if (code == 0) {
        PPSetPairingHostStatus([NSString stringWithFormat:@"STAGE 11.6.2 PAIRING COMPLETE ✅ • %@", result ?: @""]);
    } else {
        PPSetPairingHostStatus([NSString stringWithFormat:@"STAGE 11.6.2 PAIRING FAILED • %@", result ?: @""]);
    }
    return code;
}
