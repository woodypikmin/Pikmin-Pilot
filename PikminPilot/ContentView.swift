            ...
            .fileImporter(
                ...
                case .failure(let error):
                    status = fileImportTarget == .pairing
                        ? "Pairing Record 匯入失敗：\(error.localizedDescription)"
        ...
        VStack(alignment: .leading, spacing: 10) {
            HStack(spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    Text("Pikmin Pilot")
                        .font(.title2.bold())
                    Text("Stage 11.2.1 • No-VPN Transport Probe")
                        .font(.subheadline)
            ...
            Text("11.1.5 的 VPN/RSD 路線保留為已知可用 baseline。11.2.1 新增 NO-VPN SELF PROBE，只測 iPhone 能否在完全不啟動 Pikmin Tunnel 時直接碰到自己的 lockdownd；不會改動穩定的水果/花苗 loop。")
                .font(.footnote)
                .foregroundStyle(.secondary)
        }
        .padding(16)
    ...
    private var setupCard: some View {
        DisclosureGroup("設定 / 安裝狀態") {
            VStack(alignment: .leading, spacing: 12) {
                ...
                Text("Integrated Tunnel 已連線後仍需要 RPPairing Record 才能建立 phone-local RSD。新 paid Bundle ID 使用新的 App sandbox，因此舊版匯入的 Pairing Record 不會自動跟過來，只需重新匯入一次。")
                    .font(.caption)
                    .foregroundStyle(.secondary)
