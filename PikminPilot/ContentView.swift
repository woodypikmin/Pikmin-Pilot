...
struct ContentView: View {
                    ...
                    case .pairing:
                        do {
                            ...
                            if shouldResumeStart {
                                status = "STAGE 11.4.0 FIRST SETUP ✅ • Pairing Record saved • continuing START PILOT automatically…"
        ...
        VStack(alignment: .leading, spacing: 10) {
            HStack(spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    Text("Pikmin Pilot")
                        .font(.title2.bold())
                    Text("Stage 11.4.0 • One-Tap Bootstrap")
                        .font(.subheadline)
    ...
    private func startPilotOneTap() async {
        ...
        if pairing.pairingURL == nil {
            if pairing.ensureAvailableFromRecoverySources(), pairing.pairingURL != nil {
                status = "STAGE 11.4.0 ONE-TAP • Pairing recovered automatically ✅ • continuing…"
            ...
            fileImportTarget = .pairing
            status = "STAGE 11.4.0 FIRST SETUP • select the RPPairing Record once; after import START PILOT will continue automatically"
            showFileImporter = true
            return
        }
    ...
    private func startStage101Auto() async {
        ...
        busy = true
        status = "STAGE 11.4.0 START • \(runSummaryLabel) • integrated tunnel → RSD → DDI preflight → Runner → stable 10.3.1 loop"

        var tunnelNote = "integrated=not-attempted"
        do {
            try await tunnel.ensureStarted(timeoutSeconds: 10.0)
            tunnelNote = "integrated=connected"
            status = "STAGE 11.1 • integrated tunnel ✅ • probing phone-local RSD…"
        } catch {
            let diag = tunnel.diagnostics(for: error)
            tunnelNote = "integrated=unavailable"
            status = "STAGE 11.1 • integrated tunnel unavailable (\(diag)) • trying existing external LocalDevVPN path…"
        }
        ...
        guard rsd.ok else {
            busy = false
            status = "STAGE 11.1 WAITING FOR TUNNEL • \(tunnelNote) • RSD offline • integrated tunnel is unavailable; external LocalDevVPN remains a temporary fallback • \(rsd.message)"
            return
        }
        _ = pairing.backupCurrentRecordToKeychain()

        status = "STAGE 11.4.0 PREFLIGHT • RSD ✅ • checking developer services…"
        var services = await engine.probeXCTestServices()
        if !services.ok {
            status = "STAGE 11.4.0 PREFLIGHT • developer services missing after reboot • preparing Personalized DDI 27A5228h…"
            ...
            do {
                ...
                busy = false
                status = "STAGE 11.4.0 FAILED • phase=ddi-assets • \(error.localizedDescription)"
                return
            }

            status = "STAGE 11.4.0 DDI • source=\(assets.sourceLabel) • build=\(assets.buildID) • mounting through phone-local RSD…"
            ...
            guard mount.ok else {
                busy = false
                status = "STAGE 11.4.0 FAILED • phase=ddi-mount • \(mount.message)"
                return
            }

            status = "STAGE 11.4.0 DDI ✅ • \(mount.message) • rebuilding RSD…"
            let postMountRSD = await engine.probeRSD()
            guard postMountRSD.ok else {
                busy = false
                status = "STAGE 11.4.0 FAILED • phase=post-ddi-rsd • \(postMountRSD.message)"
                return
            }
            ...
            guard services.ok else {
                busy = false
                status = "STAGE 11.4.0 FAILED • phase=post-ddi-service-probe • DDI mount returned success but developer services are still absent • \(services.message)"
                return
            }
        }

        status = "STAGE 11.4.0 PREFLIGHT ✅ • RSD + DDI developer services ready • \(tunnelNote) • checking installed XCTest Runner…"
        var runner = await engine.discoverXCTestRunner()
        if !runner.ok {
            if runnerPackage.source == .embedded && runnerPackage.isEmbeddedRunnerExpired {
                busy = false
                status = "STAGE 11.1 RUNNER EXPIRED • embedded free-account provisioning has expired • \(runnerPackage.provisioningStatus) • refresh package before auto-install"
                return
            }

            if let package = runnerPackage.runnerURL {
                status = "STAGE 11.1 • Runner missing → auto-installing \(runnerPackage.sourceLabel) signed IPA via AFC + InstallationProxy…"
                let install = await engine.installXCTestRunnerIPA(localPath: package.path)
                guard install.ok else {
                    busy = false
                    status = "STAGE 11.1 FAILED • phase=runner-self-install • \(install.message)"
                    return
                }
                runner = await engine.discoverXCTestRunner()
                guard runner.ok else {
                    busy = false
                    status = "STAGE 11.1 FAILED • phase=runner-post-install-verify • \(runner.message)"
                    return
                }
                status = "STAGE 11.1 • Runner auto-bootstrap ✅ • source=\(runnerPackage.sourceLabel) • RSD ✅ • starting stable Stage 8.2.2 loop…"
            } else {
                busy = false
                status = "STAGE 11.1 PACKAGING ERROR • no installed Runner and embedded signed Runner is missing from App bundle • \(runner.message)"
                return
            }
        } else {
            status = "STAGE 11.1 • Runner ✅ • RSD ✅ • starting stable Stage 8.2.2 loop…"
        }
    ...
    private func startIntegratedTunnelOnly() async {
        busy = true
        defer { busy = false }
        status = "STAGE 11.1 TUNNEL • creating/loading paid PacketTunnelProvider configuration…"
        do {
            try await tunnel.ensureStarted(timeoutSeconds: 12.0)
            status = "STAGE 11.1 TUNNEL CONNECTED ✅ • peer=10.7.0.1/32 • next=RSD 10.7.0.1:49152"
        } catch {
            let diag = tunnel.diagnostics(for: error)
            status = "STAGE 11.1 TUNNEL FAILED • \(diag) • paid-signed tunnel failed; COPY LOG and keep external LocalDevVPN only as a temporary fallback"
        }
    }
