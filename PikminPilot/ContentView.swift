...
struct ContentView: View {
                    ...
                    case .pairing:
                        do {
                            ...
                            if shouldResumeStart {
                                status = "STAGE 11.6.1 FIRST SETUP ✅ • Pairing Record saved • continuing START PILOT automatically…"
        ...
        VStack(alignment: .leading, spacing: 10) {
            HStack(spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    Text("Pikmin Pilot")
                        .font(.title2.bold())
                    Text("Stage 11.6.1 • One-Tap Bootstrap")
                        .font(.subheadline)
    ...
    private var setupCard: some View {
        DisclosureGroup("進階 / 維修（正常使用不用打開）") {
            VStack(alignment: .leading, spacing: 12) {
                ...
                Text("11.6.1 probe：會安全保留目前可用的 Pairing recovery，只暫時隱藏 working copy，並在 iOS 26/27 都強制啟動 Pairable Host。失敗或逾時會自動恢復原本 Pairing。")
                    .font(.caption)
                    .foregroundStyle(.secondary)
    ...
    private func startPilotOneTap() async {
        ...
        if pairing.pairingURL == nil {
            if pairing.ensureAvailableFromRecoverySources(), pairing.pairingURL != nil {
                status = "STAGE 11.6.1 ONE-TAP • Pairing recovered automatically ✅ • continuing…"
            ...
            fileImportTarget = .pairing
            status = "STAGE 11.6.1 FIRST SETUP • this iOS version cannot start wireless RPPairing from zero on-device • import an existing RPPairing Record once; START PILOT will then continue automatically"
            showFileImporter = true
            return
        }
    ...
    private func runForcedPhoneLocalPairingProbe() async {
        ...
        status = "STAGE 11.6.1 PAIRING PROBE • safely holding current pairing • forcing Pairable Host on iOS \(os.majorVersion).\(os.minorVersion).\(os.patchVersion)…"
        ...
        do {
            ...
            busy = false
            status = "STAGE 11.6.1 PAIRING PROBE FAILED • could not suspend working copy • \(error.localizedDescription)"
            return
        }
        ...
        status = "STAGE 11.6.1 PAIRING PROBE • starting TCP responder + Bonjour on this phone…"
        ...
        if result.ok {
            ...
            if pairing.pairingURL != nil {
                ...
                busy = false
                status = "STAGE 11.6.1 PAIRING PROBE SUCCESS ✅ • a NEW phone-local rp_pairing_file.plist was generated on iOS \(os.majorVersion).\(os.minorVersion).\(os.patchVersion) • \(result.message)"
                return
            }
        }
        ...
        do {
            ...
            busy = false
            status = "STAGE 11.6.1 PAIRING PROBE FAILED • \(result.message) • previous working pairing restored ✅"
        } catch {
            pairing.refresh()
            busy = false
            status = "STAGE 11.6.1 PAIRING PROBE FAILED • \(result.message) • automatic working-copy restore failed: \(error.localizedDescription) • Keychain recovery was intentionally preserved"
        }
    }
    ...
    private func startPhoneLocalPairingBootstrapAndResume() async {
        ...
        busy = true
        pendingStartAfterPairingImport = false
        status = "STAGE 11.6.1 FIRST SETUP • preparing phone-local RPPairing host…"
        ...
        status = notificationsAllowed
            ? "STAGE 11.6.1 FIRST SETUP • PIN notifications ready • starting Pairable Host…"
            : "STAGE 11.6.1 FIRST SETUP • notifications unavailable; Pairable Host will still run and show the PIN in this status view"
        ...
        guard result.ok else {
            status = "STAGE 11.6.1 PAIRING FAILED • \(result.message)"
            return
        }

        pairing.refresh()
        guard pairing.pairingURL != nil else {
            status = "STAGE 11.6.1 PAIRING FAILED • responder returned success but rp_pairing_file.plist was not found"
            return
        }
        _ = pairing.backupCurrentRecordToKeychain()
        status = "STAGE 11.6.1 PAIRING COMPLETE ✅ • saved locally + secure recovery • continuing START PILOT automatically…"
        await startStage101Auto()
    }
    ...
    private func startStage101Auto() async {
        ...
        busy = true
        status = "STAGE 11.6.1 START • \(runSummaryLabel) • integrated tunnel → RSD → DDI preflight → Runner → stable 10.3.1 loop"

        var tunnelNote = "integrated=not-attempted"
        do {
            try await tunnel.ensureStarted(timeoutSeconds: 10.0)
            tunnelNote = "integrated=connected"
            status = "STAGE 11.6.1 • integrated tunnel ✅ • probing phone-local RSD…"
        } catch {
            let diag = tunnel.diagnostics(for: error)
            tunnelNote = "integrated=unavailable"
            status = "STAGE 11.6.1 • integrated tunnel unavailable (\(diag)) • trying existing external LocalDevVPN path…"
        }
        ...
        guard rsd.ok else {
            busy = false
            status = "STAGE 11.6.1 WAITING FOR TUNNEL • \(tunnelNote) • RSD offline • integrated tunnel is unavailable; external LocalDevVPN remains a temporary fallback • \(rsd.message)"
            return
        }
        _ = pairing.backupCurrentRecordToKeychain()

        status = "STAGE 11.6.1 PREFLIGHT • RSD ✅ • checking developer services…"
        var services = await engine.probeXCTestServices()
        if !services.ok {
            status = "STAGE 11.6.1 PREFLIGHT • developer services missing after reboot • preparing Personalized DDI 27A5228h…"
            ...
            do {
                ...
                busy = false
                status = "STAGE 11.6.1 FAILED • phase=ddi-assets • \(error.localizedDescription)"
                return
            }

            status = "STAGE 11.6.1 DDI • source=\(assets.sourceLabel) • build=\(assets.buildID) • mounting through phone-local RSD…"
            ...
            guard mount.ok else {
                busy = false
                status = "STAGE 11.6.1 FAILED • phase=ddi-mount • \(mount.message)"
                return
            }

            status = "STAGE 11.6.1 DDI ✅ • \(mount.message) • rebuilding RSD…"
            let postMountRSD = await engine.probeRSD()
            guard postMountRSD.ok else {
                busy = false
                status = "STAGE 11.6.1 FAILED • phase=post-ddi-rsd • \(postMountRSD.message)"
                return
            }
            ...
            guard services.ok else {
                busy = false
                status = "STAGE 11.6.1 FAILED • phase=post-ddi-service-probe • DDI mount returned success but developer services are still absent • \(services.message)"
                return
            }
        }

        status = "STAGE 11.6.1 PREFLIGHT ✅ • RSD + DDI developer services ready • \(tunnelNote) • synchronizing XCTest Runner…"
        var runner = await engine.discoverXCTestRunner()

        if runnerPackage.source == .embedded && runnerPackage.isEmbeddedRunnerExpired {
            busy = false
            status = "STAGE 11.6.1 RUNNER EXPIRED • embedded provisioning expired • \(runnerPackage.provisioningStatus)"
            return
        }

        // Stage 11.6.1 is host/pairing-only. Keep the proven Stage 11.5.3
        ...
        if needsRunnerSync {
            guard let package = runnerPackage.runnerURL else {
                busy = false
                status = "STAGE 11.6.1 PACKAGING ERROR • embedded signed Runner missing"
                return
            }
            status = "STAGE 11.6.1 RUNNER SYNC • hostBuild=\(hostBuild) • runnerRevision=\(runnerRevision) • source=\(runnerPackage.sourceLabel) • installing/upgrading…"
            let install = await engine.installXCTestRunnerIPA(localPath: package.path)
            guard install.ok else {
                busy = false
                status = "STAGE 11.6.1 FAILED • phase=runner-sync • \(install.message)"
                return
            }
            runner = await engine.discoverXCTestRunner()
            guard runner.ok else {
                busy = false
                status = "STAGE 11.6.1 FAILED • phase=runner-sync-verify • \(runner.message)"
                return
            }
            UserDefaults.standard.set(runnerRevision, forKey: runnerSyncKey)
            status = "STAGE 11.6.1 RUNNER SYNC ✅ • hostBuild=\(hostBuild) • runnerRevision=\(runnerRevision) • verified current embedded Runner"
        } else {
            status = "STAGE 11.6.1 RUNNER ✅ • hostBuild=\(hostBuild) • runnerRevision=\(runnerRevision) • Stage 11.5.3 Runner preserved"
        }
    ...
    private func startIntegratedTunnelOnly() async {
        busy = true
        defer { busy = false }
        status = "STAGE 11.6.1 TUNNEL • creating/loading paid PacketTunnelProvider configuration…"
        do {
            try await tunnel.ensureStarted(timeoutSeconds: 12.0)
            status = "STAGE 11.6.1 TUNNEL CONNECTED ✅ • peer=10.7.0.1/32 • next=RSD 10.7.0.1:49152"
        } catch {
            let diag = tunnel.diagnostics(for: error)
            status = "STAGE 11.6.1 TUNNEL FAILED • \(diag) • paid-signed tunnel failed; COPY LOG and keep external LocalDevVPN only as a temporary fallback"
        }
    }
    ...
    private func restartIntegratedTunnel() async {
        ...
        defer { busy = false }
        rsdReady = false
        status = "STAGE 11.6.1 • hard-restarting PacketTunnelProvider session…"
        do {
            try await tunnel.restartFresh(timeoutSeconds: 15.0)
            status = "STAGE 11.6.1 TUNNEL FRESH ✅ • device=10.7.0.0 • fake=10.7.0.1 • next=RSD 10.7.0.1:49152"
        } catch {
            status = "STAGE 11.6.1 TUNNEL RESTART FAILED • \(tunnel.diagnostics(for: error))"
        }
    }
