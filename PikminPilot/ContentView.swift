...
struct ContentView: View {
                    ...
                    case .pairing:
                        do {
                            ...
                            if shouldResumeStart {
                                status = "STAGE 11.5.4.18 FIRST SETUP ✅ • Pairing Record saved • continuing START PILOT automatically…"
        ...
        VStack(alignment: .leading, spacing: 10) {
            HStack(spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    Text("Pikmin Pilot")
                        .font(.title2.bold())
                    Text("Stage 11.5.4.18 • iPad Foreground Recovery • 11.5.3 baseline")
                        .font(.subheadline)
    ...
    private func startPilotOneTap() async {
        ...
        if pairing.pairingURL == nil {
            if pairing.ensureAvailableFromRecoverySources(), pairing.pairingURL != nil {
                status = "STAGE 11.5.4.18 ONE-TAP • Pairing recovered automatically ✅ • continuing…"
            ...
            fileImportTarget = .pairing
            status = "STAGE 11.5.4.18 FIRST SETUP • select the RPPairing Record once; after import START PILOT will continue automatically"
            showFileImporter = true
            return
        }
    ...
    private func startStage101Auto() async {
        ...
        // Stage 11.5.4.18 step 3: an RSD session was established while Airplane
        ...
        if !wifiAvailableAtStart, cellularSessionReadyForRestore, let persistentEngine = cellularPersistentEngine {
            busy = true
            status = "STAGE 11.5.4.18 CELLULAR ESCAPE STEP 3 • verifying the already-open RSD session after 4G/5G restore • no :49152 reconnect…"
            ...
            guard health.ok else {
                ...
                busy = false
                status = "STAGE 11.5.4.18 PERSISTENT SESSION DID NOT SURVIVE CELLULAR RESTORE ❌ • \(health.message) • keep Integrated Tunnel on, enable Airplane Mode again, return and press START PILOT"
                return
            }
            ...
            startPersistentAutomation(
                ...
                banner: "STAGE 11.5.4.18 CELLULAR ESCAPE READY ✅ • persistent RSD survived 4G/5G restore • starting without :49152 reconnect"
            )
            return
        }

        // Stage 11.5.4.18 WARM SESSION: once cellular bootstrap has succeeded,
        ...
        if !wifiAvailableAtStart, cellularInterfaceAvailable(), let persistentEngine = cellularPersistentEngine {
            busy = true
            status = "STAGE 11.5.4.18 WARM CELLULAR SESSION • checking existing persistent RSD before raw RPPairing…"
            let health = await persistentEngine.probePersistentSessionHealth()
            if health.ok {
                busy = false
                rsdReady = true
                startPersistentAutomation(
                    ...
                    banner: "STAGE 11.5.4.18 WARM CELLULAR DIRECT ✅ • existing RSD session alive • Airplane bootstrap skipped for this run"
                )
                return
            }
            await persistentEngine.closePersistentSession()
            cellularPersistentEngine = nil
            busy = false
            status = "STAGE 11.5.4.18 WARM SESSION EXPIRED • live RSD no longer usable • falling back to normal bootstrap detection"
        }

        busy = true
        status = "STAGE 11.5.4.18 START • \(runSummaryLabel) • 11.5.3 baseline transport"

        var tunnelNote = "integrated=not-attempted"
        do {
            try await tunnel.ensureStarted(timeoutSeconds: 10.0)
            tunnelNote = "integrated=connected"
            status = "STAGE 11.5.4.18 • integrated tunnel ✅ • probing phone-local RSD…"
        } catch {
            let diag = tunnel.diagnostics(for: error)
            tunnelNote = "integrated=unavailable"
            status = "STAGE 11.5.4.18 • integrated tunnel unavailable (\(diag)) • probing existing path…"
        }
        ...
        guard rsd.ok else {
            ...
            if wifiAvailableAtStart {
                cellularBootstrapArmed = false
                status = "STAGE 11.5.4.18 WIFI BASELINE RSD FAILED ❌ • \(tunnelNote) • Wi-Fi=en0 active • \(rsd.message) • cellular workaround NOT entered"
                return
            }
            ...
            if cellularInterfaceAvailable() {
                cellularBootstrapArmed = true
                status = "STAGE 11.5.4.18 CELLULAR ESCAPE STEP 1 • \(tunnelNote) • raw RPPairing unavailable with cellular active • \(rsd.message) • KEEP Pikmin Pilot Integrated Tunnel ON → turn Airplane Mode ON → return to Pikmin Pilot → press START PILOT again"
                return
            }

            if cellularBootstrapArmed {
                status = "STAGE 11.5.4.18 AIRPLANE BOOTSTRAP FAILED ❌ • no Wi-Fi / no cellular interface • \(tunnelNote) • \(rsd.message) • keep Integrated Tunnel on and press START PILOT again, or return to Wi-Fi for direct mode"
                return
            }

            status = "STAGE 11.5.4.18 RSD OFFLINE ❌ • no active Wi-Fi/cellular bootstrap path • \(tunnelNote) • \(rsd.message)"
            return
        }
        ...
        if cellularBootstrapArmed {
            status = "STAGE 11.5.4.18 CELLULAR ESCAPE STEP 2 • RPPairing reachable with temporary Airplane Mode ✅ • pinning one persistent RSD session…"
            let persistent = await engine.openPersistentSession()
            guard persistent.ok else {
                busy = false
                status = "STAGE 11.5.4.18 CELLULAR ESCAPE FAILED ❌ • phase=persistent-session-create • \(persistent.message)"
                return
            }
            usingPersistentCellularSession = true
            status = "STAGE 11.5.4.18 PERSISTENT RSD PINNED ✅ • \(persistent.message) • preflighting DDI/Runner before cellular restore…"
        }

        status = "STAGE 11.5.4.18 PREFLIGHT • RSD ✅ • checking developer services…"
        var services = await engine.probeXCTestServices()
        if !services.ok {
            status = "STAGE 11.5.4.18 PREFLIGHT • developer services missing • preparing Personalized DDI 27A5228h…"
            ...
            do {
                ...
                busy = false
                status = "STAGE 11.5.4.18 FAILED • phase=ddi-assets • if Airplane Mode is on, run once on Wi-Fi beforehand so DDI assets are cached • \(error.localizedDescription)"
                return
            }

            status = "STAGE 11.5.4.18 DDI • source=\(assets.sourceLabel) • build=\(assets.buildID) • mounting…"
            ...
            guard mount.ok else {
                if usingPersistentCellularSession { await engine.closePersistentSession() }
                busy = false
                status = "STAGE 11.5.4.18 FAILED • phase=ddi-mount • \(mount.message)"
                return
            }
            ...
            if usingPersistentCellularSession {
                ...
                guard reopened.ok else {
                    busy = false
                    status = "STAGE 11.5.4.18 FAILED • phase=post-ddi-persistent-reopen • \(reopened.message)"
                    return
                }
            } else {
                ...
                guard postMountRSD.ok else {
                    busy = false
                    status = "STAGE 11.5.4.18 FAILED • phase=post-ddi-rsd • \(postMountRSD.message)"
                    return
                }
            }
            ...
            guard services.ok else {
                if usingPersistentCellularSession { await engine.closePersistentSession() }
                busy = false
                status = "STAGE 11.5.4.18 FAILED • phase=post-ddi-service-probe • \(services.message)"
                return
            }
        }

        status = "STAGE 11.5.4.18 PREFLIGHT ✅ • RSD + DDI ready • synchronizing XCTest Runner…"
        var runner = await engine.discoverXCTestRunner()

        if runnerPackage.source == .embedded && runnerPackage.isEmbeddedRunnerExpired {
            if usingPersistentCellularSession { await engine.closePersistentSession() }
            busy = false
            status = "STAGE 11.5.4.18 RUNNER EXPIRED • embedded provisioning expired • \(runnerPackage.provisioningStatus)"
            return
        }
        ...
        if needsRunnerSync {
            guard let package = runnerPackage.runnerURL else {
                if usingPersistentCellularSession { await engine.closePersistentSession() }
                busy = false
                status = "STAGE 11.5.4.18 PACKAGING ERROR • embedded signed Runner missing"
                return
            }
            status = "STAGE 11.5.4.18 RUNNER SYNC • hostBuild=\(hostBuild) • installing/upgrading…"
            let install = await engine.installXCTestRunnerIPA(localPath: package.path)
            guard install.ok else {
                if usingPersistentCellularSession { await engine.closePersistentSession() }
                busy = false
                status = "STAGE 11.5.4.18 FAILED • phase=runner-sync • \(install.message)"
                return
            }
            ...
            guard runner.ok else {
                if usingPersistentCellularSession { await engine.closePersistentSession() }
                busy = false
                status = "STAGE 11.5.4.18 FAILED • phase=runner-sync-verify • \(runner.message)"
                return
            }
        ...
        if usingPersistentCellularSession {
            ...
            busy = false
            rsdReady = true
            status = "STAGE 11.5.4.18 CELLULAR ESCAPE STEP 2 READY ✅ • PERSISTENT RSD + DDI + RUNNER READY • turn Airplane Mode OFF • AUTO-RESUME is armed; no third START should be needed"
    ...
    private func armCellularAutoResume() {
        ...
        cellularAutoResumeTask = Task { @MainActor in
            ...
            for _ in 0..<240 { // up to ~2 minutes; manual START remains a fallback
                ...
                if sawCellularDown && cellularUp,
                   let engine = cellularPersistentEngine {
                    status = "STAGE 11.5.4.18 AUTO CELLULAR RESUME • pdp_ip restored ✅ • validating pinned RSD session…"
                    ...
                    if health.ok {
                        ...
                        startPersistentAutomation(
                            ...
                            banner: "STAGE 11.5.4.18 AUTO CELLULAR RESUME READY ✅ • 4G/5G returned • starting Pilot automatically • no third START"
                        )
                    } else {
                        await engine.closePersistentSession()
                        cellularPersistentEngine = nil
                        ...
                        cellularBootstrapArmed = true
                        busy = false
                        status = "STAGE 11.5.4.18 AUTO RESUME FAILED ❌ • pinned RSD did not survive cellular restore • \(health.message) • repeat Airplane bootstrap"
                    }
                    return
                }
            ...
            if cellularSessionReadyForRestore {
                status = "STAGE 11.5.4.18 AUTO RESUME WAITING • cellular interface was not observed in time • session is still pinned; press START PILOT manually after 4G/5G returns"
            }
        }
    }
    ...
    private func startIntegratedTunnelOnly() async {
        busy = true
        defer { busy = false }
        status = "STAGE 11.5.4.18 TUNNEL • creating/loading paid PacketTunnelProvider configuration…"
        do {
            try await tunnel.ensureStarted(timeoutSeconds: 12.0)
            status = "STAGE 11.5.4.18 TUNNEL CONNECTED ✅ • peer=10.7.0.1 • next=RSD 10.7.0.1:49152"
        } catch {
            let diag = tunnel.diagnostics(for: error)
            status = "STAGE 11.5.4.18 TUNNEL FAILED • \(diag) • paid-signed tunnel failed; COPY LOG and keep external LocalDevVPN only as a temporary fallback"
        }
    }
