...
type ReadyCallback = extern "C" fn(
    service_identifier: *const c_char,
    port: u16,
    txt_records: *const c_char,
);
type PinCallback = extern "C" fn(pin: *const c_char);
...
/// Pikmin Pilot Stage 11.6.1 on-device RPPairing responder.
///
...
pub unsafe extern "C" fn pilot_pairing_host_accept(
    ...
    let result: Result<String, String> = run_sync_local(async move {
        ...
        if let Some(callback) = ready_callback {
            ...
            callback(service_c.as_ptr(), port, txt_c.as_ptr());
        }
